...
example_json = {
    ...
    "sectionImageDir":"/nas4/data/S3_Run1_Jarvis/processed/Low_res/S3_Run1_Jarvis/Stitched_DAPI_1_dropped/sections_at_0.05",
    "polygon_dir":"/nas4/data/S3_Run1_Jarvis/processed/shape_polygons",
    "isHorizontal":False
}
...
def find_section_boundaries(r,
                    ...
                    plot=False,
                    isHorizontal=False):
    thousands = np.floor(int(z/1000))
    hundreds = np.floor(int(z)%1000)/100

    filepath = os.path.join(image_cache_dir,'%03d'%thousands,'%d'%hundreds,'%.1f.tiff'%z)
